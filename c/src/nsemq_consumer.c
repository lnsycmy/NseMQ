...
extern TopicList topic_list_;        // defined in nsemq_base.c
...
ErrorCode nsemq_consumer_close() {
    ...
    while (curNode) {
        ...
        if(err_ != ERR_NO_ERROR){
            err_ = rd_kafka_last_error();
            sprintf(strtemp_, "Failed to stop consuming topic(%s):%s", curNode->topic_name, (char *)rd_kafka_err2str(err_));
            nsemq_write_error(consumer_, strtemp_);
        }
        curNode = curNode->next;
    }
